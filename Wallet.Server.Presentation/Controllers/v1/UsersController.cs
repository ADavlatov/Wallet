    ...
    ILogger<UsersController> logger) : ControllerBase
{
    ...
    {
        logger.LogInformation("Начало запроса на регистрацию. Username: {Username}.", request.Username);
        var validationResult = await new AuthRequestValidator().ValidateAsync(request, cancellationToken);
        if (!validationResult.IsValid)
        {
            logger.LogWarning("Ошибка валидации при регистрации. Username: {Username}.", request.Username);
            return BadRequest(validationResult.Errors);
        }

        var result = await usersService.SignUp(request.Username, request.Password, cancellationToken);
        logger.LogInformation("Запрос на регистрацию завершен. " +
                              "Username: {Username}, UserId: {UserId}.", request.Username, result.UserId);
    ...
    {
        logger.LogInformation("Начало запроса на авторизацию. Username: {Username}.", request.Username);
        var validationResult = await new AuthRequestValidator().ValidateAsync(request, cancellationToken);
        if (!validationResult.IsValid)
        {
            logger.LogWarning("Ошибка валидации при авторизации. Username: {Username}.", request.Username);
            return BadRequest(validationResult.Errors);
        }

        var result = await usersService.SignIn(request.Username, request.Password, cancellationToken);
        logger.LogInformation("Запрос на авторизацию завершен. " +
                              "Username: {Username}, UserId: {UserId}.", request.Username, result.UserId);
    ...
    public async Task<IActionResult> RefreshTokens([FromBody] RefreshTokensRequest request)
    {
        ...
        var result = await usersService.RefreshTokens(request.RefreshToken);
        logger.LogInformation("Запрос на обновление токенов завершен. UserId: {UserId}.", result.UserId);
        return Ok(result);
    }
    ...
    {
        logger.LogInformation("Начало запроса на получение пользователя по имени. " +
                              "Username: {Username}.", request.Username);

        var validationResult = await new GetUserByUsernameRequestValidator().ValidateAsync(request, cancellationToken);
        if (!validationResult.IsValid)
        {
            logger.LogWarning("Ошибка валидации при получении пользователя по имени. " +
                              "Username: {Username}.", request.Username);
        ...
        logger.LogInformation("Запрос на получение пользователя по имени завершен. " +
                              "Username: {Username}, UserId: {Id}.", request.Username, result.Id);
    ...
    {
        logger.LogInformation("Начало запроса на получение пользователя по ID. UserId: {UserId}.", userId);
        var result = await usersService.GetUserById(userId, cancellationToken);
        logger.LogInformation("Запрос на получение пользователя по ID завершен. UserId: {UserId}.", userId);
        return Ok(result);
    }
    ...
    public async Task<IActionResult> UpdateUser([FromBody] UpdateUserRequest request,
        CancellationToken cancellationToken)
    {
        logger.LogInformation("Начало запроса на обновление пользователя. UserId: {UserId}.", request.UserId);
        await usersService.UpdateUser(request.UserId, request.Username, request.Password, cancellationToken);
        logger.LogInformation("Запрос на обновление пользователя завершен. UserId: {request.UserId}.", request.UserId);
        return Ok();
    }
    ...
    {
        logger.LogInformation("Начало запроса на удаление пользователя. UserId: {UserId}.", userId);
        await usersService.DeleteUser(userId, cancellationToken);
        logger.LogInformation("Запрос на удаление пользователя завершен. UserId: {UserId}.", userId);
        return Ok();
    }
    ...
    {
        logger.LogInformation("Начало запроса на получение API ключа. UserId: {UserId}.", request.UserId);
        var result = await usersService.GetApiKey(request.UserId, cancellationToken);
        logger.LogInformation("Запрос на получение API ключа завершен. UserId: {UserId}.", request.UserId);
        return Ok(result);
    }
    ...
    public async Task<IActionResult> UpdateApiKey([FromBody] UpdateApiKeyRequest request,
        CancellationToken cancellationToken)
    {
        logger.LogInformation("Начало запроса на обновление API ключа. UserId: {UserId}.", request.UserId);
        await usersService.UpdateApiKey(request.UserId, cancellationToken);
        logger.LogInformation("Запрос на обновление API ключа завершен. UserId: {UserId}.", request.UserId);
        return Ok();
    }
        ...
        CancellationToken cancellationToken)
    {
        logger.LogInformation("Начало запроса на валидацию API ключа. " +
                              "ApiKey: {ApiKey}, " +
                              "TelegramUserId: {TelegramUserId}", request.ApiKey, request.TelegramUserId);

        var validationResult = await new ValidateApiKeyRequestValidator().ValidateAsync(request, cancellationToken);
        if (!validationResult.IsValid)
        {
            logger.LogWarning("Ошибка валидации при валидации API ключа. ApiKey: {ApiKey}.", request.ApiKey);
            return BadRequest(validationResult.Errors);
        }

        await usersService.ValidateApiKey(request.ApiKey, request.TelegramUserId, cancellationToken);
        logger.LogInformation("Запроса на валидацию API ключа завершен. " +
                              "ApiKey: {request.ApiKey}, " +
                              "TelegramUserId: {request.TelegramUserId}", request.ApiKey, request.TelegramUserId);
